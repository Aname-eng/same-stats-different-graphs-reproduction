...
class NankaiOptimizerV2:
    def __init__(
        ...
        if "layer_name" in df_slots.columns:
            self.slot_layer_names = df_slots["layer_name"].to_numpy()
            self.layer_names = list(pd.unique(df_slots["layer_name"]))
        else:
            self.layer_names = list(GEOMETRY_LAYERS)
            self.slot_layer_names = np.array([self.layer_names[min(i, len(self.layer_names)-1)] for i in self.slot_layer_ids])
        ...
        # Initial assignment
        self.assignment = fast_spatial_assignment(self.points, self.slots)
    ...
    def save_snapshot(self, step: int, is_final: bool = False):
        ...
        df_out = pd.DataFrame({
            ...
            "assigned_layer_name": self.slot_layer_names[self.assignment]
        })
        df_out.to_csv(snap_csv, index=False)

        # 1. Pure scatter plot
        fig, ax = plt.subplots(figsize=(7, 7))
        dot_s = 8 if self.n > 1500 else 14
        ax.scatter(self.points[:, 0], self.points[:, 1], s=dot_s, color="#1f77b4", edgecolors="none", alpha=0.9)
        ax.set_xlim(0, 100)
        ax.set_ylim(0, 100)
        ax.set_aspect("equal")
        ...
        # 2. Diagnostic plot
        fig, ax = plt.subplots(figsize=(7, 7))
        ax.scatter(self.slots[:, 0], self.slots[:, 1], s=6, color="#d3d3d3", alpha=0.4, label="Target Slots")
        colors = ["#1f77b4", "#2ca02c", "#ff7f0e", "#d62728", "#9467bd", "#8c564b", "#e377c2"]
        for l_id, l_name in enumerate(self.layer_names):
            m = (assigned_lids == l_id)
            if np.any(m):
                ax.scatter(self.points[m, 0], self.points[m, 1], s=dot_s, color=colors[l_id % len(colors)], label=f"{l_name} ({np.sum(m)})", alpha=0.85)
        ax.set_xlim(0, 100)
        ax.set_ylim(0, 100)
        ax.set_aspect("equal")
    ...
    def optimize(self, total_steps: int = 60000, snapshot_interval: int = 15000):
        ...
        for step in range(1, total_steps + 1):
            ...
            # Adaptive proposal selection: batch of disjoint triads for high N acceleration
            batch_size = max(1, min(24, int(round(self.n / 600))))
            errs = np.sum((self.points - self.slots[self.assignment])**2, axis=1)
            err_probs = errs / errs.sum()

            try:
                primary = self.rng.choice(self.n, size=batch_size, replace=False, p=err_probs)
                other = self.rng.choice(self.n, size=2 * batch_size, replace=False)
            ...
            if accepted_in_batch > 0:
                if signature2(cand_pts, ddof=1) == REF_SIGNATURE:
                    ...
                    self.current_energy += batch_delta
...
def main():
    ...
    # Auto-scale reassign interval for very large N
    df_s = pd.read_csv(args.seed_csv)
    n_pts = len(df_s)
    default_reassign = 1000 if n_pts > 4000 else (600 if n_pts > 1500 else 400)
    reassign_int = args.reassign_interval or default_reassign
    ...
    opt = NankaiOptimizerV2(
        ...
        output_dir=out_dir,
        reassign_interval=reassign_int
    )
