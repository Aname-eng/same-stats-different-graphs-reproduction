...
def show_scatter(df, xlim=(-5, 105), ylim=(-5, 105), color="black", marker="o", reg_fit=False):
    # Seaborn 0.13 makes ``data`` the first positional parameter; use explicit
    # keyword arguments to retain the behavior of the original 2017 code.
...
def do_single_run(start_dataset, target, iterations=100000, decimals=2, num_frames=100):
    ...
    temp = run_pattern(df, target, iters=iterations, decimals=decimals, num_frames=num_frames)
    return temp
